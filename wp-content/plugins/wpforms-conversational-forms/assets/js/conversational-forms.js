/* global wpforms_conversational_forms, MobileDetect */
...
var WPFormsConversationalForms = window.WPFormsConversationalForms || ( function( document, window, $ ) {
	...
	scrollControl = {
		...
		next: function() {
			...
			// Select(dropdown) fields with conditional logic never trigger 'wpformsProcessConditionals' event
			// and have to be processed without binding app.scroll.processNext() to this event.
			if ( app.fields.active && app.fields.active.$el.hasClass( 'wpforms-field-select' ) ) {
			...
			if ( app.fields.active && app.fields.active.$el.hasClass( 'wpforms-conditional-trigger' ) ) {
				...
				$( window.document ).one( 'wpformsProcessConditionals', app.scroll.processNext );
